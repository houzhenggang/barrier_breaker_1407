...
for new_d in $new_module_dirs ; do
	...
	if [ "$patch_openwrt" = "1" ] ; then
		...
		echo "" >>../package/kernel/linux/modules/netfilter.mk
		echo "" >>../package/kernel/linux/modules/netfilter.mk
		echo "define KernelPackage/ipt-$lower_name" >>../package/kernel/linux/modules/netfilter.mk
		echo "  SUBMENU:=\$(NF_MENU)" >>../package/kernel/linux/modules/netfilter.mk
		echo "  TITLE:=$lower_name" >>../package/kernel/linux/modules/netfilter.mk
		echo "  KCONFIG:=\$(KCONFIG_IPT_$upper_name)" >>../package/kernel/linux/modules/netfilter.mk
		echo "  FILES:=\$(LINUX_DIR)/net/ipv4/netfilter/*$lower_name*.\$(LINUX_KMOD_SUFFIX)" >>../package/kernel/linux/modules/netfilter.mk
		echo "  AUTOLOAD:=\$(call AutoLoad,45,\$(notdir \$(IPT_$upper_name-m)))" >>../package/kernel/linux/modules/netfilter.mk
		echo "	DEPENDS:= kmod-ipt-core" >>../package/kernel/linux/modules/netfilter.mk
		echo "endef" >>../package/kernel/linux/modules/netfilter.mk
		echo "\$(eval \$(call KernelPackage,ipt-$lower_name))" >>../package/kernel/linux/modules/netfilter.mk

	
		#add OpenWrt package definition for iptables extension
		echo "" >>../package/network/utils/iptables/Makefile 
		echo "" >>../package/network/utils/iptables/Makefile 
		echo "define Package/iptables-mod-$lower_name" >>../package/network/utils/iptables/Makefile 
		echo "\$(call Package/iptables/Module, +kmod-ipt-$lower_name)" >>../package/network/utils/iptables/Makefile 
		echo "  TITLE:=$lower_name" >>../package/network/utils/iptables/Makefile 
		echo "endef" >>../package/network/utils/iptables/Makefile 
		echo "\$(eval \$(call BuildPlugin,iptables-mod-$lower_name,\$(IPT_$upper_name-m)))" >>../package/network/utils/iptables/Makefile 
...
if [ "$patch_kernel" = 1 ] ; then	
	...
	rm -f ../package/network/utils/iptables/patches/650-custom_netfilter_match_modules.patch 2>/dev/null
	cd iptables.new
